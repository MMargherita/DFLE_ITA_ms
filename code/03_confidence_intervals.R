rm(list = ls())
libraries <- list("data.table", "VGAM", "tidyverse","doParallel","foreach")
...
for (yr in all_yrs) {
  ...
  # available in the workspace due to this setup
  boot_fx <- function() {
    #-------data and models ----------------------------------
    
    dati <-  as.data.frame(rbind(sub2[sample(nrow(sub2), replace = T),],
    ...
    # Models for corrections (using smoothed prev according to same model)
  ...
  expectancies <-
    foreach(
      ...
      .combine = 'rbind',
      .packages = c('VGAM', 'Formula', 'data.table', 'tidyverse'),
  ...
  ci <- function(theta) {
    quantile(theta, probs = c(.025, .975), type = 8)
  }
  ...
  write.csv(CICH, file = file.path(dir_out, paste0("expectancies_CI_", yr, ".csv")))
