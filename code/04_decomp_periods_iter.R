...
for (s in c("men", "women")) {
  for (p in 1:3) {
    ...
    Umat_left       <- Umat_left %>% filter(sex == s & i == iter) %>% select(-c(sex,i))
    Umat_left       <- Umat_left %>% column_to_rownames(var="to")
    
    
    Umat_right_name <- paste0("sex_matrix_lt_2023_ita_", period_right[p], ".rds")
    Umat_right      <- readRDS(paste0("data/data_out/",Umat_right_name))
    Umat_right      <- Umat_right %>% filter(sex == s & i == iter) %>% select(-c(sex,i))
    Umat_right      <- Umat_right %>% column_to_rownames(var="to")
    ...
    file.name <-
      paste0("cc_", period_left[p], "_", period_right[p], "_", s, "_DFLE_", iter,".rda")
    save(cc_p_attrition, file = file.path("data/data_out/cc_p_iter/", file.name))
