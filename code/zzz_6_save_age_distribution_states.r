...
for (i in myr) {
  ...
  tr_format[, age_g := cut(
    age,
    seq(49, 83, 5),
    labels = paste(seq(50, 75, 5), seq(54, 79, 5), sep = "-"),
    include.lowest = F,
    right = T
  )]
  ...
  weights_f <-
    prop.table(xtabs(~ age_g + from, data = subset(tr_format, gender == 2)), 1)
  weights_f <- as.data.frame(weights_f)
  weights_f[, "year"] <- i
  weights_f[, "sex"] <- "F"
  
  weights_m <-
    prop.table(xtabs(~ age_g + from, data = subset(tr_format, gender == 1)), 1)
