...
#north
tp_mf_North_04 <- readRDS("data/tp_mf_North_2004.rds")  %>% 
...
# tp 
tp_mf_Centre_04 <- readRDS("data/tp_mf_Centre_2004.rds")  %>% 
...
#just death probs
qx_mf_Centre_04 <- tp_mf_Centre_04 %>% 
  group_by(gender,rep,age1,from) %>% 
  mutate(probs = 1-(sum(probs)),
         to = "D") %>% 
  select(year,gender,rep,age1,from,age2,to,probs) %>% 
...
# tp 
tp_mf_South_04 <- readRDS("data/tp_mf_South_2004.rds")  %>% 
...
data_trans_death %>%
  ...
  ggtitle("Transition probability to death from ITSILC compared to qx from ISTAT - 2004")

# ggsave(filename = "plot/tp_death_qx_04.png",
#        device = "png", dpi = 200,
#        width = 15, height = 8)
